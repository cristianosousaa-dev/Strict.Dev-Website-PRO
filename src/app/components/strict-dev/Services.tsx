...
export function Services() {
  ...
  return (
    <section className="bg-white dark:bg-[#0a0a0a] py-12 md:py-20" id="services" aria-labelledby="services-title">
      <div className="container mx-auto px-3 md:px-6 max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-12 pb-4 md:pb-8 border-b border-neutral-100 dark:border-[#1a1a1a]">
            <div>
                ...
                <h2 id="services-title" className="text-xl md:text-3xl lg:text-4xl font-bold text-neutral-900 dark:text-neutral-100 tracking-tight">{t.services.title}</h2>
            </div>
        ...
        >
            {serviceIcons.map((Icon, idx) => (
                ...
                <Link
                    to={servicePaths[idx]}
                    className="relative p-3 md:p-5 bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-[#1a1a1a] shadow-sm overflow-hidden hover:border-[#2f5e50] flex flex-col h-full cursor-pointer"
                    aria-label={t.services.items[idx].title}
                >
                    {/* Badge no topo direito */}
                    <div className="absolute top-2 md:top-2 right-2 md:right-2">
                      <span className={`px-1.5 md:px-2 py-0.5 md:py-1 text-[8px] md:text-[9px] font-bold ${serviceBadges[idx].color} border rounded uppercase tracking-wide`}>
                        {t.services.badges[idx]}
                      </span>
                    </div>
                    
                    <div className="text-neutral-400 dark:text-neutral-600 hover:text-[#2f5e50] mb-2 md:mb-3">
                      <Icon className="w-5 md:w-6 h-5 md:h-6 stroke-[1.5]" aria-hidden="true" />
                    </div>
                    ...
                    <div className="pt-2 md:pt-2 border-t border-neutral-200 dark:border-[#1a1a1a] mt-auto">
                      <p className="text-[8px] md:text-[9px] text-[#2f5e50] font-bold uppercase tracking-wider">
                        {t.services.pricing}
                      </p>
                    </div>
                </Link>
