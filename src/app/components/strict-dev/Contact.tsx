...
import { WEB3FORMS_ACCESS_KEY } from "../../../config/keys";
...
export function Contact() {
  ...
  const validateField = (field: string, value: string) => {
    ...
    switch (field) {
      case "name":
        if (!value.trim()) {
          error = t.contact.formRequired;
        } else if (value.trim().length < 2) {
          error = t.contact.formNameShort;
        } else {
          isValid = true;
        }
        break;
      ...
      case "email":
        if (!value.trim()) {
          ...
          error = t.contact.formEmailInvalid;
        } else {
          isValid = true;
        }
        break;
      ...
      case "requirements":
        if (!value.trim()) {
          ...
          error = t.contact.formRequirementsShort;
        } else {
          isValid = true;
        }
        break;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!securityCheck.valid) {
      ...
      if (securityCheck.reason === "rate_limit_exceeded") {
        toast.error(t.contact.rateLimitTitle || t.toast.rateLimitTitle, {
          description: t.contact.rateLimitDesc || t.toast.rateLimitDesc,
          duration: 5000,
    ...
    try {
      ...
      if (!WEB3FORMS_ACCESS_KEY || WEB3FORMS_ACCESS_KEY === "YOUR_ACCESS_KEY_HERE") {
        throw new Error("Falta configurar WEB3FORMS_ACCESS_KEY em src/config/keys.ts");
      }
      ...
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: `Novo Contacto - ${formData.name}`,
  ...
  return (
    <section className="bg-white dark:bg-[#0a0a0a] py-12 md:py-20" id="contact" aria-labelledby="contact-title">
      <div className="container mx-auto px-3 md:px-6 max-w-7xl">
        <div className="lg:hidden mb-8">
          <span className="text-[#2f5e50] font-bold text-[0.5625rem] uppercase tracking-widest mb-2 block">
            {t.contact.label}
          </span>
          <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100 tracking-tight mb-3" id="contact-title">
            {t.contact.title}
          </h2>
          ...
          <motion.div className="order-3 lg:order-1" variants={leftVariants}>
            ...
            <div className="space-y-5 md:space-y-6 border-t border-neutral-100 dark:border-[#1a1a1a] pt-5 md:pt-6">
              <div className="group cursor-pointer">
                <p className="text-[0.5625rem] md:text-[9px] font-bold uppercase tracking-widest text-neutral-400 mb-0.5 md:mb-1">
                  {t.contact.email}
                </p>
                <a
              ...
              <div className="border-t border-neutral-100 dark:border-[#1a1a1a] pt-5 md:pt-6">
                <div className="flex items-start gap-2.5 md:gap-3">
                  <Clock className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#2f5e50] mt-0.5 flex-shrink-0" aria-hidden="true" />
                  <div>
                    <p className="text-[0.5625rem] md:text-[9px] font-bold uppercase tracking-widest text-neutral-900 dark:text-neutral-100 mb-1.5 md:mb-2">
          ...
          >
            <div className="mb-6 md:mb-8 flex flex-wrap gap-3 md:gap-4 pb-5 md:pb-6 border-b border-neutral-100 dark:border-[#1a1a1a]">
              <div className="flex items-center gap-2">
                <Lock className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#2f5e50] flex-shrink-0" aria-hidden="true" />
                <span className="text-[0.5625rem] md:text-[10px] text-neutral-600 dark:text-neutral-400 font-medium">
                  {t.contact.reassurance1}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#2f5e50] flex-shrink-0" aria-hidden="true" />
                <span className="text-[0.5625rem] md:text-[10px] text-neutral-600 dark:text-neutral-400 font-medium">
                  {t.contact.reassurance2}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Shield className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#2f5e50] flex-shrink-0" aria-hidden="true" />
                <span className="text-[0.5625rem] md:text-[10px] text-neutral-600 dark:text-neutral-400 font-medium">
            ...
            <form className="space-y-5 md:space-y-6" onSubmit={handleSubmit} noValidate>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1 group relative">
                  <label htmlFor="contact-name" className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500 group-focus-within:text-[#2f5e50] transition-colors">
                    {t.contact.formName} *
                  </label>
                  <div className="relative">
                    ...
                    />
                    {fieldValid.name && (
                      <CheckCircle className="absolute right-1 top-1/2 -translate-y-1/2 w-4 h-4 text-[#2f5e50]" aria-hidden="true" />
                    )}
                  </div>
                  {fieldErrors.name && (
                ...
                <div className="space-y-1 group">
                  <label htmlFor="contact-company" className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500 group-focus-within:text-[#2f5e50] transition-colors">
              ...
              <div className="space-y-1 group relative">
                <label htmlFor="contact-email" className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500 group-focus-within:text-[#2f5e50] transition-colors">
                  {t.contact.formEmail} *
                </label>
                <div className="relative">
                  <Input
              ...
              <div className="space-y-1 group relative">
                <label htmlFor="contact-requirements" className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500 group-focus-within:text-[#2f5e50] transition-colors">
                ...
                <div className="relative">
                  ...
                  />
                  {fieldValid.requirements && (
                    <CheckCircle className="absolute right-1 top-3 w-4 h-4 text-[#2f5e50]" aria-hidden="true" />
                  )}
                </div>
                {fieldErrors.requirements && (
                  <p className="text-[10px] text-red-500 mt-1" role="alert">
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      ...
                      <Send className="w-3 h-3 ml-2 opacity-70" aria-hidden="true" />
                    </>
                  )}
                </Button>
              </div>
