...
export function Hero({ onShowAnimation, shouldHideCore, showAnimation, onCloseAnimation }: HeroProps) {
  ...
  // ============================================
  // CINEMATIC SPLIT DISSOLVE - page-level useScroll()
  // DESKTOP: Hero → esquerda | NeuralCore → direita
  // MOBILE: fade vertical suave (sem split horizontal)
  // Sem target ref (evita "Target ref not hydrated")
  // ============================================
  const { scrollY } = useScroll();

  // --- DESKTOP: HERO desliza para a ESQUERDA + fade ---
  const heroXDesktop = useTransform(scrollY, [80, 900], [0, -280], { clamp: true });
  const heroOpacityDesktop = useTransform(scrollY, [80, 850], [1, 0], { clamp: true });
  const heroScaleDesktop = useTransform(scrollY, [80, 900], [1, 0.97], { clamp: true });
  const heroBlurDesktop = useTransform(scrollY, [150, 800], [0, 8], { clamp: true });

  // --- MOBILE: 3D perspective tilt (inclina para trás + afunda + dissolve) ---
  const heroYMobile = useTransform(scrollY, [40, 550], [0, 70], { clamp: true });
  const heroOpacityMobile = useTransform(scrollY, [40, 500], [1, 0], { clamp: true });
  const heroScaleMobile = useTransform(scrollY, [40, 550], [1, 0.88], { clamp: true });
  const heroBlurMobile = useTransform(scrollY, [100, 450], [0, 4], { clamp: true });
  const heroRotateXMobile = useTransform(scrollY, [40, 550], [0, 12], { clamp: true });

  // --- Selecionar valores por breakpoint ---
  const heroX = isMobile ? 0 : heroXDesktop;
  const heroY = isMobile ? heroYMobile : 0;
  const heroOpacity = isMobile ? heroOpacityMobile : heroOpacityDesktop;
  const heroScale = isMobile ? heroScaleMobile : heroScaleDesktop;
  const heroBlurVal = isMobile ? heroBlurMobile : heroBlurDesktop;
  const heroBlur = useMotionTemplate`blur(${heroBlurVal}px)`;
  const heroRotateX = isMobile ? heroRotateXMobile : 0;
  
  // --- NEURAL CORE (desktop only): desliza para a DIREITA + fade + blur ---
  const neuralX = useTransform(scrollY, [80, 900], [0, 320], { clamp: true });
  const neuralOpacity = useTransform(scrollY, [80, 850], [1, 0], { clamp: true });
  const neuralScale = useTransform(scrollY, [80, 900], [1, 0.97], { clamp: true });
  const neuralBlurVal = useTransform(scrollY, [150, 800], [0, 8], { clamp: true });
  const neuralBlur = useMotionTemplate`blur(${neuralBlurVal}px)`;

  // --- SCROLL INDICATOR: desaparece primeiro ---
  const indicatorOpacity = useTransform(scrollY, [30, 250], [1, 0], { clamp: true });

  return (
    <section className="relative pt-20 pb-10 md:pt-24 md:pb-16 lg:pt-32 bg-white dark:bg-[#0a0a0a] min-h-screen flex flex-col justify-center overflow-hidden border-b border-neutral-200 dark:border-[#1a1a1a]" aria-labelledby="hero-home-title">
      
      {/* NEURAL CORE - desliza para a DIREITA ao scroll */}
      {!isMobile && (
        <motion.div
      ...
      {/* BUILD ANIMATION - Z-45 (não dissolve) */}
      {!isMobile && (
      ...
      {/* ============================================ */}
      {/* HERO CONTENT - desliza para a ESQUERDA       */}
      {/* Wrapper exterior: scroll dissolve            */}
      {/* Filhos interiores: entrance animations       */}
      {/* ============================================ */}
      <motion.div
        style={{
          x: heroX,
      ...
      >
        <div className="container mx-auto max-w-7xl px-3 md:px-6 pointer-events-auto">
          ...
          <div className="relative py-6 md:py-0 pointer-events-auto z-40">
              ...
              <h1 id="hero-home-title" className="text-[1.75rem] leading-[1.1] md:text-4xl lg:text-6xl font-bold text-neutral-900 dark:text-neutral-100 md:leading-[0.95] tracking-tight mb-3 md:mb-6">
                  {t.hero.title1} <br/>
              ...
              <div className="flex flex-col md:flex-row gap-2.5 md:gap-4 mb-6 md:mb-12">
                  ...
                  >
                      <a href="#contact" aria-label={t.hero.ctaPrimary}>
                          {t.hero.ctaPrimary}
                          <ArrowDown className="w-3 h-3 ml-2 -rotate-90 group-hover:rotate-0 transition-transform duration-300" aria-hidden="true" />
                      </a>
                  </Button>
                   <Button 
                      asChild
              ...
              <div className="flex flex-col md:flex-row gap-2 md:gap-6 pt-3.5 md:pt-6">
                  <div className="flex items-center gap-1.5 md:gap-2">
                      <CheckCircle className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#2f5e50] flex-shrink-0" aria-hidden="true" />
                  ...
                  <div className="flex items-center gap-1.5 md:gap-2">
                      <BrainCircuit className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#2f5e50] flex-shrink-0" aria-hidden="true" />
      ...
      {/* ============================================ */}
      {/* SCROLL INDICATOR - fade out rapido           */}
      {/* ============================================ */}
      ...
      >
        {/* Texto "SCROLL" */}
        <span className="text-[0.5625rem] md:text-[9px] uppercase tracking-[0.2em] font-bold text-neutral-400 dark:text-neutral-500 mb-1">
          Scroll
        </span>
        ...
        <div className="relative flex flex-col items-center">
          ...
          >
            <ChevronDown className="w-4 h-4 md:w-5 md:h-5 text-[#2f5e50] opacity-70 group-hover:opacity-100 transition-opacity" strokeWidth={2.5} aria-hidden="true" />
          </motion.div>
        </div>
      </motion.div>
    </section>
