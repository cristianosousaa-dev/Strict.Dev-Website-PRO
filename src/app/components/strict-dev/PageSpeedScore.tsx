...
function ScoreRing({ baseScore, isRange, label, delay, size, strokeWidth }: ScoreRingProps) {
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      <span className="text-[8px] md:text-[9px] text-neutral-500 dark:text-neutral-400 uppercase tracking-wider text-center leading-tight max-w-[56px] md:max-w-[68px]">
        {label}
      </span>
    </motion.div>
  );
...
export function PageSpeedScore() {
  ...
  // Guard: if translations don't have pagespeed, don't render
  const ps = t?.hero?.pagespeed;
  if (!ps) return null;
  ...
  const scores: Omit<ScoreRingProps, "size" | "strokeWidth" | "delay">[] = [
    { baseScore: 90,  isRange: true,  label: ps.performance },
    { baseScore: 95,  isRange: true,  label: ps.accessibility },
    { baseScore: 100, isRange: false, label: ps.bestPractices },
    { baseScore: 100, isRange: false, label: ps.seo },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3 md:mb-4">
        ...
        <span className="text-[0.5625rem] md:text-[9px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500">
          {ps.label}
        </span>
        {/* Note badge: Mobile & Desktop */}
        <span className="text-[8px] md:text-[8px] font-bold uppercase tracking-wider text-[#2f5e50] bg-[#2f5e50]/8 px-1.5 py-0.5 border border-[#2f5e50]/15">
          {ps.note}
        </span>
      </div>
      ...
      <motion.a
        ...
        aria-label={`${ps.verify} (${language === "pt" ? "abre numa nova janela" : "opens in a new window"})`}
        className="inline-flex items-center gap-1 mt-3 md:mt-4 text-[0.5625rem] md:text-[9px] text-[#2f5e50] hover:text-[#234539] font-bold uppercase tracking-widest cursor-pointer group transition-colors duration-200"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        <ArrowUpRight className="w-2.5 h-2.5 md:w-3 md:h-3 opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-200" aria-hidden="true" />
      </motion.a>
    </motion.div>
  );
}